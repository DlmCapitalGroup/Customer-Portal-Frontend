...
import Table3 from "../../components/EnquiriesTable";
...
const Enquiries = () => {
    ...
    const fetchCustomers = useCallback((pageNumber: number) => {
        setLoading(true);
        devInstance
            .get("/Transaction/GetAllInquiries", {
                params: {
                    pageNumber: pageNumber,
                },
            })
            .then((res: any) => {
                setEnquiries(res?.data?.data?.pageItems);
    ...
    const filteredSearch = enquiries?.filter((customer: any) => {
        return (
            customer?.fullName
                ?.toString()
                ?.toLowerCase()
